...
### 데이터 종류가 늘어나면 doPost에 kind 필드로 분기하고, 각 kind는 자기 스키마의 탭만 만든다
매입/매출/통장/카드처럼 처음엔 한 가지였던 동기화 대상이 늘어난다(플랫폼 정산내역 등). 매번 새 엔드포인트를
만들지 말고, payload에 `kind` 필드를 하나 추가해서 `doPost` 안에서 분기하고, 새 kind마다 자기만의
HEADERS/KEY_COL 상수와 ingest 함수를 하나씩 추가하는 식으로 확장하는 게 깔끔하다. 탭 이름 규칙도
`{사업자} {연도} {종류}`로 통일해두면, 요약 집계 쪽에서 정규식 하나로 모든 kind를 순회할 수 있다.
공통으로 하는 뒷정리(참고용 매핑 탭 생성, 요약 재계산 등)는 각 ingest 끝에서 공용 `afterWrite(ss)` 하나만
부르게 묶어두면 kind가 늘어도 빠뜨리지 않는다.

### 여러 탭을 가로질러 월별 집계할 때: 쓴 타입과 읽히는 타입이 다를 수 있다
날짜를 `"2026-01-17"` 같은 문자열로 `setValues`에 넘겨도, 셀 서식이 자동으로 날짜로 인식되면 시트에는
진짜 Date로 저장된다. 그래서 나중에 `getValues()`로 다시 읽으면 문자열이 아니라 JS `Date` 객체가 나올 수
있다 — 쓸 때 타입과 읽을 때 타입이 다르다는 뜻. 여러 탭(세금계산서 탭, 플랫폼 매출 탭 등)을 순회하며 "연월"
단위로 묶는 집계 함수를 짤 때는 반드시 두 경우를 다 처리하는 헬퍼를 통과시킬 것:
```javascript
...
```
`v.slice(0,7)`만 믿고 짜면, Date로 저장된 셀에서 `"Mon Jan 17 00:00..."` 같은 문자열이 나와 연월 키가
전부 깨지는데 에러 없이 조용히 잘못된 결과만 나와서 발견하기 어렵다.
